...
if sys.hexversion < 3 << 24:
    print(
        "bibgraphes ne fonctionne qu'avec idle3 (python3), pas avec idle (python)")
    sys.exit(1)
...
def _getGraphViz():
    if __plateforme == 'Windows':
        ...
        if liste == []:
            print(
                "Graphviz non trouve, la fonction dessiner utilisera le site web GraphvizOnline")
            return ""
        path = liste[0]
        return path[0:-7]
...
def _errMaj(wrong, right):
    raise Exception(
        "Attention aux majuscules/minuscules: la fonction " + wrong + " n'existe pas, c'est la fonction " + right + " qui existe")


def _errS(wrong, right):
    raise Exception(
        "Attention aux s: la fonction " + wrong + " n'existe pas, c'est la fonction " + right + " qui existe")


def _errOrtho(wrong, right):
    raise Exception(
        "Attention à l'orthographe: la fonction " + wrong + " n'existe pas, c'est la fonction " + right + " qui existe")


def _errAccents(wrong, right):
    raise Exception(
        "Attention à ne pas utiliser d'accent dans Python : la fonction " + wrong + " n'existe pas, c'est la fonction " + right + " qui existe")
...
class __c_edge:
    ...
    def __repr__(self):
        return "<arete: '%s' %s--%s>" % (
        self.label, self.start.label, self.end.label)
...
def sommetNom(G: graphe, etiquette: str) -> sommet:
    ...
    for s in listeSommets(G):
        if s.label.lower() == etiquette.lower():
            raise Exception("le graphe " + nomGraphe(
                G) + " ne possède pas de sommet d'étiquette '" + etiquette + "'." \
...
def sommetVoisin(s: sommet, a: arete) -> sommet:
    ...
    if a.end == s:
        return a.start
    raise Exception("\n\nle sommet '" + nomSommet(
        s) + "' n'est pas une extremite de l'arete ('" + nomSommet(
        a.start) + "', '" + nomSommet(a.end) + "').")
...
def __verif_type_graphe(G):
    if G.__class__.__name__ != '__c_graph':
        if isinstance(G, str):
            raise TypeError(
                "'" + G + "' est une chaine de caracteres alors que la fonction attend un graphe. Peut-etre voulez-vous utiliser la fonction ouvrirGraphe(G)?")
        if isinstance(G, _io.TextIOWrapper):
            raise TypeError("'" + str(
                G) + "' est un fichier ouvert alors que la fonction attend un graphe. Peut-etre voulez-vous utiliser la fonction ouvrirGraphe(G)?")
...
def __verif_type_sommet(s):
    if s.__class__.__name__ != '__c_node':
        if isinstance(s, str):
            raise TypeError(
                "'" + s + "' est une chaine de caracteres alors que la fonction attend un sommet. Peut-etre voulez-vous utiliser la fonction sommetNom(G, etiquette)?")
...
def __verif_type_nomfichier(s):
    if not isinstance(s, str):
        raise __ErreurParametre(s,
                                'un nom de fichier, par exemple "tgv2005.dot"')
...
def __verif_type_couleur(s):
    if not isinstance(s, str):
        raise __ErreurParametre(s,
                                'une chaine de caracteres représentant une couleur comme par exemple : "red", "green", "blue", "white", "cyan" ou "yellow".')
...
def __dotify(G, etiquettesAretes=True, colormark='Black', suffixe='dot',
             path=None):
    if __plateforme != 'Windows':
    ...
    if path:
        ...
        graph_dot = 'tmp/' + nom_graphe + '-' + str(
            __dotify.num) + '.' + suffixe
    ...
    for s in G.nodes:
        ...
        if s.mark:
            ...
            entoure = 1
        if s.color:
            if s.color[0:4] == "dark" or s.color in ["black", "blue", "blue1",
                                                     "blue2", "blue3", "blue4",
                                                     "mediumblue", "blueviolet",
                                                     "indigo", "navy",
                                                     "navyblue", "purple4",
                                                     "brown4", "gray", "grey"]:
                fontcolor = "white"
                ...
                fontcolor = "black"
            f.write(
                '  %s %s [style = filled, peripheries = %s, fillcolor = %s, fontcolor = %s, color = %s];\n' %
...
def dessinerGraphe(G: graphe, etiquettesAretes=False, algo='dot',
                   colormark='Black') -> None:
    """ dessine le graphe G
    Exemple d'utilisation :
    ...
    __verif_type_graphe(G)
    if nbSommets(G) > 100:
        print("Attention, le graphe a " + str(
            nbSommets(G)) + " sommets, le dessin va prendre du temps...")
    if __pathGraphviz == "":
...
def dessinerGraphe2(G: graphe, etiquettesAretes=False, algo='dot',
                    colormark='Black') -> None:
    graph_dot = __dotify(G, etiquettesAretes, colormark)
...
def _mot(s, debut):
    while debut < len(s) and (
            s[debut] == ' ' or s[debut] == '\t' or s[debut] == '\n' or s[
        debut] == '\r'):
        debut += 1
    ...
    if s[debut] == '"':
        ...
        while fin < len(s):
            ...
            fin += 1
        raise SyntaxError(
            "Fichier incorrect: \" not terminé à la fin du fichier")

    charclass = _charclass(s[fin])
    while fin < len(s) and (
            s[fin] != ' ' and s[fin] != '\t' and s[fin] != '\n' and s[
        fin] != '\r'):
        if s[fin] == '#':
        ...
        if s[fin] == '"':
            raise SyntaxError(
                "Fichier incorrect: \" au milieu d'un mot à " + str(debut))
        fin += 1
...
def _attributs(s, i):
    ...
    while nom != ']':
        if nom == "":
            raise SyntaxError(
                "Fichier incorrect: pas de crochet fermant à " + str(i))
        if nom == ",":
            nom, i = _mot(s, i)
        egal, i = _mot(s, i)
        if egal != '=':
            raise SyntaxError(
                "Fichier incorrect: trouvé " + egal + " au lieu d'un '=' à " + str(
                    i))
        val, i = _mot(s, i)
...
def _litgrapheDOT(s, i):
    ...
    if accolade != "{":
        raise SyntaxError(
            "Fichier incorrect: trouvé " + accolade + " au lieu d'une accolade ouvrante à " + str(
                i))
    ...
    while mot != "}":
        ...
        if mot == "":
            raise SyntaxError(
                "Fichier incorrect: pas d'accolade fermante terminale à la fin du fichier")
        ...
        if mot in ("graph", "node", "edge"):
            ...
            if crochet != '[':
                raise SyntaxError(
                    "Fichier incorrect: trouvé " + crochet + " au lieu d'un crochet ouvrant à " + str(
                        i) + ' ' + str(j))
            i = j
            ...
            if equal != '=':
                raise SyntaxError(
                    "Fichier inccorect: pour " + mot + ", trouvé " + equal + " au lieu d'un = à " + str(
                        i))
            i = j
            val, i = _mot(s, i)
            defattr += [mot + '=' + val]
            ...
            _, chemins_sousgraphe, couleurs_sousgraphe, marks_sousgraphe, nodeattr_sousgraphe, edgeattr_sousgraphe, defattr_sousgraphe, i = _litgrapheDOT(
                s, i)
            ...
            if mot2 == '[':
                ...
                raise SyntaxError(
                    "Fichier non supporté: trouvé " + mot2 + " à " + str(i))
        while mot == ';':
            mot, i = _mot(s, i)
        ...
        if mot == "directed":
            ...
            while mot != ']':
                if mot == 'id':
                    ...
                    raise SyntaxError(
                        'mot-clé de sommet ' + mot + ' non supporté à ' + str(
                            i))
            if nom == "":
                nom = str(ID)
            # print("sommet "+nom)
...
def _litgraphePAJ(s, i):
    ...
    if mot in ("Edges", "Arcs"):
        raise SyntaxError(
            'Attendu Edges ou Arcs, trouvé ' + mot + ' à la place')
...
def ouvrirGraphe(nom: str) -> graphe:
    ...
    try:
        ...
        raise Exception(
            "Attention, le fichier " + nom + " n'existe pas, peut-être le nom est mal écrit, ou bien ce fichier n'est pas dans le même répertoire que le fichier .py ?")
    s = f.read()
    i = 0
    ...
    if graph == "Creator":
        ...
        if graph not in ("digraph", "graph"):
            raise SyntaxError(
                "Fichier graphe de type " + graph + " non supporté")

        nom, chemins, couleurs, marks, nodeattr, edgeattr, defattr, i = _litgrapheDOT(
            s, i)
    # print("construction")
